...
@Component({
  selector: 'app-you-trucker-offers',
  templateUrl: './you-trucker-offers.component.html',
  styleUrls: ['./you-trucker-offers.component.scss']
})
export class YouTruckerOffersComponent implements OnInit {
  ...
  sendRespond(rejectOrConfirm: boolean): void {
    ...
    if (rejectOrConfirm) {
      this.CommunicationService.changeNotificationStatus(this.notificationId, 'confirmed', this.chat).then(() => {
        ...
        this.CommunicationService.changeTruckerOfferStatus(this.offerId, 'in-work').then(() => {
        })
      });
    } else {
